...
def binary_evaluation(
    X: Union[pd.DataFrame, np.ndarray], y: Union[pd.Series, np.ndarray], mod
) -> pd.DataFrame:
    ...
    bal_acc = metrics.balanced_accuracy_score(y, pred)
    prec = metrics.precision_score(y, pred)
    rec = metrics.recall_score(y, pred)
    f1 = metrics.f1_score(y, pred)

    df_result = pd.DataFrame(columns=["metrics", "value"])
    df_result["metrics"] = ["accuracy", "balanced acc", "precision", "recall", "f1"]
    df_result["value"] = [acc, bal_acc, prec, rec, f1]

    return df_result


def binary_cross_evaluate(
    ...
    **kwargs
    ...
    result_dict = cross_validate(
        clf, X, y, cv=num_fold, scoring=["accuracy", "f1"], **kwargs
    )
