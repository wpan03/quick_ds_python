...
def get_lg_coef(df: pd.DataFrame, mod_lg) -> pd.DataFrame:
    ...
    df_coef = pd.DataFrame({"name": df.columns, "value": mod_lg.coef_.reshape(-1)})
...
def get_feature_imp(df: pd.DataFrame, mod_rf) -> pd.DataFrame:
    ...
    result = pd.DataFrame({"name": df.columns, "score": mod_rf.feature_importances_})

    return result.sort_values("score", ascending=False, ignore_index=True)


def get_permute_imp_df(
    mod,
    X: pd.DataFrame,
...
def cross_entropy_loss(
    y_true: np.ndarray, y_pred_1: np.ndarray, eps: float = 1e-8
) -> np.ndarray:
...
def get_binary_error_analysis(
    X: pd.DataFrame, y: pd.Series, mod, only_show_error: bool = False
) -> pd.DataFrame:
    ...
    if only_show_error:
        df_result = df_result[df_result["pred_class"] != df_result["actual_class"]]
...
def plot_single_instance_explaination(mod_rf, instance: pd.DataFrame) -> None:
    ...
    fig = go.Figure(
        go.Waterfall(
            orientation="v",
            measure=["absolute"] + len(instance.columns) * ["relative"] + ["total"],
            x=["start"] + list(instance.columns) + ["final_prediction"],
            y=[bias[0][which_class]]
            + list(contributions[0][:, which_class])
            + [prediction[0][which_class]],
        )
    )
    fig.show()
