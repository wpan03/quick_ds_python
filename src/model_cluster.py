...
def cluster_selection_plot(df: pd.DataFrame, mod_list: List,
                           cluster_range: List) -> None:
...
def summary_plot(df: pd.DataFrame, mod) -> None:
    ...
    for i in range(mod.n_clusters):
        ...
        axs[i].scatter(x=df_summary.iloc[i, 2:].values,
                       y=list(df.columns),
                       color='royalblue')
